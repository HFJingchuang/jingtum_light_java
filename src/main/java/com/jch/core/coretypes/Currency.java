...
import com.jch.core.coretypes.hash.Hash160;
import com.jch.core.coretypes.uint.UInt64;
...
//				return XRP;//SWT
				...
				/*
				 * if (!(value.length() == 3)) { // if (!value.matches("[A-Z0-9]{3}")) { throw
				 * new RuntimeException("Currency code must be 3 characters"); }
				 */
	...
	/*
	 * The following are static methods, legacy from when there was no usage of
	 * Currency objects, just String with "XRP" ambiguity.
	 */
	public static byte[] encodeCurrency(String currencyCode) {
		byte[] currencyBytes = new byte[20];
		/*
		 * currencyBytes[12] = (byte) currencyCode.codePointAt(0); currencyBytes[13] =
		 * (byte) currencyCode.codePointAt(1); currencyBytes[14] = (byte)
		 * currencyCode.codePointAt(2);
		 */
	...
	public static String getCurrencyCodeFromTLCBytes(byte[] bytes) {
		...
		for (i = 0; i < 20; i++) {
			zeroInNonCurrencyBytes = zeroInNonCurrencyBytes && ((offset <= i && i <= (offset + size)) || // currency
																											// bytes (0
																											// or any
																											// other)
		...
		/*
		 * for (i = 0; i < 20; i++) { zeroInNonCurrencyBytes = zeroInNonCurrencyBytes &&
		 * ((i == 12 || i == 13 || i == 14) || // currency bytes (0 or any other)
		 * bytes[i] == 0); // non currency bytes (0) } if (zeroInNonCurrencyBytes) {
		 * return isoCodeFromBytesAndOffset(bytes, 12); } else { throw new
		 * IllegalStateException("Currency is invalid"); }
		 */
	}
	...
	private static String isoCodeFromBytesAndOffset(byte[] bytes, int offset) {
		...
		/*
		 * char a = charFrom(bytes, offset); char b = charFrom(bytes, offset + 1); char
		 * c = charFrom(bytes, offset + 2); return "" + a + b + c;
		 */
	}
}
