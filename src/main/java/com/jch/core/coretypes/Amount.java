...
/**
 * In ripple, amounts are either XRP, the native currency, or an IOU of a given
 * currency as issued by a designated account.
 */
public class Amount extends Number implements SerializedType, Comparable<Amount> {
	...
	// public static final Amount NEUTRAL_ZERO = new Amount(Currency.NEUTRAL,
	// AccountID.NEUTRAL);
	 ...
	 * 
	 * All operations return amounts of the same currency/issuer as the first
	 * operand.
	 * 
	 * eg.
	 * 
	 ...
	 * 
	 * Divide and multiply are equivalent to the javascript ripple-lib ratio_human
	 * and product_human.
	 * 
	 */
	...
	/**
	 * @return Amount The real native unit is a drop, one million of which are an
	 *         XRP. We want `one` unit at XRP scale (1e6 drops), or if it's an IOU,
	 *         just `one`.
	 */
	public Amount one() {
		if (isNative()) {
	...
	public static class Translator extends TypeTranslator<Amount> {
		...
		public Amount fromString(String s) {
			...
			return com.jch.core.coretypes.Amount.fromString(s);
		}
	...
	public static Amount fromIOUString(String val) {
		...
		if (split.length == 1) {
			throw new RuntimeException(
					"IOU string must be in the form number/currencyString or number/currencyString/issuerString");
	...
	public static BigDecimal roundValue(BigDecimal value, boolean nativeSrc) {
		...
		return value.setScale(nativeSrc ? MAXIMUM_NATIVE_SCALE : MAXIMUM_IOU_PRECISION - i,
