...
import com.jch.core.coretypes.hash.prefixes.LedgerSpace;
import com.jch.core.coretypes.uint.UInt32;
import com.jch.core.coretypes.uint.UInt64;

import static com.jch.core.coretypes.hash.HalfSha512.prefixed256;
import static java.util.Collections.sort;

public class Index {
	private static Hash256 createBookBase(Issue pays, Issue gets) {
		return prefixed256(LedgerSpace.bookDir).add(pays.currency()).add(gets.currency()).add(pays.issuer())
				.add(gets.issuer()).finish();
	}
	...
	public static Hash256 quality(Hash256 index, UInt64 quality) {
		...
		if (quality != null)
			System.arraycopy(quality.toBytes(), 0, qi, 24, 8);
	 ...
	 * @param nodeIndex nullable LowNode, HighNode, OwnerNode, BookNode etc defining
	 *                  a `page` number.
	 *
	 * @return A hash of rootIndex and nodeIndex when nodeIndex is non default else
	 *         the rootIndex. This hash is used as an index for the next
	 *         DirectoryNode page.
	 */
	public static Hash256 directoryNode(Hash256 rootIndex, UInt64 nodeIndex) {
		...
		return prefixed256(LedgerSpace.dirNode).add(rootIndex).add(nodeIndex).finish();
	}
	...
	public static Hash256 ledgerHashes(long prev) {
		return prefixed256(LedgerSpace.skipList).add(new UInt32(prev >> 16)).finish();
	}
